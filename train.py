...
# parser.add_argument('--ratio', type=float, nargs='+', default=[1./5., 3./5.], metavar='RATIO',
...
def train_one_epoch(
    ...
    for batch_idx, (input, target, angel_target) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            weight = 0.2
            weight_branch = 0.8
            loss = weight*loss_fn(output, target)+weight_branch*loss_fn(angel_output,angel_target) 
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    ...
    losses_m = AverageMeter()
    top1_m = AverageMeter()
    angel_top1m = AverageMeter()
    classmae_m = AverageMeter()
    angelmae_m = AverageMeter()
    model.eval()
    ...
    with torch.no_grad():
        for batch_idx, (input, target, angel_target) in enumerate(loader):
            ...
            if isinstance(output, (tuple, list)):
                output = output[0]
                angel_output = angel_output[0]
            # augmentation reduction
            reduce_factor = args.tta
            if reduce_factor > 1:
                output = output.unfold(0, reduce_factor, reduce_factor).mean(dim=2)
                target = target[0:target.size(0):reduce_factor]
                angel_target = angel_target[0:angel_target.size(0):reduce_factor]
            batch_size = output.size(0)
            angel_batch = angel_output.size(0)
            output[output>=0.5] = 1
            output[output<0.5] = 0
            ...
            loss = loss_fn(output, target)+loss_fn(angel_output,angel_target)
            target = torch.sum(target,dim=1)[:,0]
            angel_target = torch.sum(angel_target,dim=1)[:,0]
            classmae = abs(predict_class-target).sum()
            classmae = classmae/target.size(0)
            angelmae = abs(predict_angel-angel_target).sum()
            angelmae = angelmae/target.size(0)
            
         
            
            num_same = (predict_class == target).sum().item()
            angel_same = (predict_angel == angel_target).sum().item()
            ...
            if args.distributed:
                ...
                angel_acc1 = reduce_tensor(angel_acc1, args.world_size)
            ...
            angel_top1m.update(angel_acc1.item(), angel_batch)
            classmae_m.update(classmae.item(), output.size(0))
            angelmae_m.update(angelmae.item(), angel_output.size(0))
            ...
            if args.local_rank == 0 and (last_batch or batch_idx % args.log_interval == 0):
                ...
                # log_path = os.path.join(output_dir,'log.txt')
                f = open(log_path, 'a')
                f.write(str('{0}: [{1:>4d}/{2}]  '
                    'Time: {batch_time.val:.3f} ({batch_time.avg:.3f})  '
                    'Loss: {loss.val:>7.4f} ({loss.avg:>6.4f})  '
                    'Acc@1: {top1.val:>7.4f} ({top1.avg:>7.4f})  '
                    'Angel_acc@1: {angel1.val:>7.4f} ({angel1.avg:>7.4f}) '
                    .format(
                        log_name, batch_idx, last_idx, batch_time=batch_time_m,
                        loss=losses_m, top1=top1_m,angel1=angel_top1m)) + '\n')
                f.close()
                _logger.info(
                    '{0}: [{1:>4d}/{2}]  '
                    'Time: {batch_time.val:.3f} ({batch_time.avg:.3f})  '
    ...
    metrics = OrderedDict([('loss', losses_m.avg), ('top1', top1_m.avg),('angel_top1', angel_top1m.avg),('classmae', classmae_m.avg),('angelmae',angelmae_m.avg)])
